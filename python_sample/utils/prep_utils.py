...
def import_object_from_file(file_path, object_name):
    ...
    - The imported object.
    """
    module_name = f"imported_module_{object_name}"
    spec = importlib.util.spec_from_file_location(module_name, file_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)

    # Dynamically access the imported object from the module
    imported_object = getattr(module, object_name)
    return imported_object


def projectPoints(xyz, K, M = None):
    """
...
def get_norm_params(dataloader, from_tensor = False):
    """
    ...
    for i, sample in tqdm(enumerate(dataloader)):
        data = sample["image_raw"] if not from_tensor else sample
        batch_samples = data.size(0) if not from_tensor else 1
        #print(type(data))
        #size1 = data.size[1] if from_tensor else data.size(1)
        
        data = data.view(batch_samples, data.size(1 if not from_tensor else 0), -1)
